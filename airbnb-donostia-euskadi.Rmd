...
Por comparar, en otras ciudades, el porcentaje de alojamientos con licencia es Barcelona (`r format(round(100*barcelonaLicenciaPerCent, 1), nsmall = 1)` de un total de `r nbarcelona`), Madrid (0%) y Mallorca (0%), según datos de insideairbnb.
...
ggplot(donostia) +  geom_bar(mapping = aes(x = beds, fill = withlicense))
ggplot(donostia) +  geom_bar(mapping = aes(x = property_type, fill = beds))
...
ggplot(donostia[donostia$withlicense==TRUE,]) + geom_bar(mapping = aes(x = property_type)) + coord_flip() + labs(title= "Donostia con licencia" ) #cambiar por variable withlicense
