...
Es interesante repasar en qué fechas el scraper de datahippo.org revisó (campo "revised") los anuncios, esto es, volvió a guardar la información porque los consideró activso (hay `r nrow(reviseddates)` fechas distintas de "revised"). En la siguiente lista se indican las fechas y el número de anuncios con esa fecha:
```{r reviseddates_table, echo = FALSE, warning=FALSE, message=FALSE}
kable(reviseddates,caption = "")
```
...
labs(
  ...
  subtitle = paste(nrow(local_activo_f[host.id %in% n_alojamientos[1,]$host.id,]), #Revisar   
...
labs(
  ...
  subtitle = paste(nrow(local_activo_f[host.id %in% n_alojamientos[2,]$host.id,]),
                   " alojamientos en ", local_activo_name,  sep = ""),
  caption = caption_2
  ) +guides(colour = guide_legend(override.aes = list(size=3))) 
```
