---
title: "Efecto Airbnb en Donostia - San Sebastián"
...
Este informe se ha generado automáticamente con R a partir del [script de Rmarkdown](https://github.com/montera34/airbneuskadi/tree/master/analisis/preanalisis-airbnb.Rmd) disponible en el [repositorio airbnbeuskadi](https://github.com/montera34/airbneuskadi).
...
En **`r local_activo_name`** se han publicado `r format(nrow(local_activo), big.mark='.')` anuncios de Airbnb desde que Datahippo guarda registro, que han tenido capacidad para `r format(sum(na.omit(local_activo$accommodates)), big.mark='.') ` plazas (accommodates). 
